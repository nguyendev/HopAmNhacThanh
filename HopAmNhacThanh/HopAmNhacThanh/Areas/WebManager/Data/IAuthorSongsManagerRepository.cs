...
{
    public interface IAuthorSongsManagerRepository
    {
        Task<AuthorSong> Get(long? id);
        bool Exists(long id);
        Task<IndexAuthorSongViewModel> GetAll(string sortOrder, string searchString,
    int? page, int? pageSize);
        Task<bool> Create(CreateAuthorSongViewModel model, ApplicationUser user);
        Task<EditAuthorSongViewModel> GetEdit(long? ID);
        Task Update(EditAuthorSongViewModel model);
        Task<AuthorSong> Details(long? id);
        Task Delete(long id);
