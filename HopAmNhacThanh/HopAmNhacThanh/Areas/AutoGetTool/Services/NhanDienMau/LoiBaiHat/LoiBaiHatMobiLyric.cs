...
{
    public class LoiBaiHatMobiLyric
    {
        public static bool Check(HtmlDocument doc)///div
        {
            var nodeP = doc.DocumentNode.SelectSingleNode(".//*[@id='container']/div[1]/div[5]/div/p");
            if (nodeP == null)
                return false;
        ...
        public static NhanDienLyricViewModel GetContent(HtmlDocument doc, string url)
        {
            ...
            var nodeLyric = doc.DocumentNode.SelectSingleNode(".//*[@id='container']/div[1]/div[5]/div/p");
            lyrics = nodeLyric.InnerText;

            return new NhanDienLyricViewModel(url, lyrics, "LoiBaiHat.Mobi");
        }
    }
}