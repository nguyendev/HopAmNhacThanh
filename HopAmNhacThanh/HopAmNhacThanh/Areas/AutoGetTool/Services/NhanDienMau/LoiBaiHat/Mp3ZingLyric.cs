...
{
    public class ThuVienTinLanhLyric
    {
        public static bool Check(HtmlDocument doc)///div
        {
            var nodeP = doc.DocumentNode.SelectSingleNode(".//*[@id='article_content']/div[2]");
            if (nodeP == null)
                return false;
        ...
        public static NhanDienLyricViewModel GetContent(HtmlDocument doc, string url)
        {
            ...
            var nodeLyric = doc.DocumentNode.SelectSingleNode(".//*[@id='article_content']/div[2]");
            lyrics = nodeLyric.InnerText;

            return new NhanDienLyricViewModel(url, lyrics, "Loibaihat.Biz");
        }
    }
}