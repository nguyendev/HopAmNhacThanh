...
{
    public class AuthorSongRepository : IAuthorSongRepository
    {
        private readonly ApplicationDbContext _context;
        public AuthorSongRepository(ApplicationDbContext context)
        {
            _context = context;
        }
        public async Task<CommonListViewModel> GetListAuthorSong(int page, int pageSize)
        {
            var applicationDbContext = from s in _context.AuthorSong
                ...
                .OrderByDescending(p => p.CreateDT)
                                 select s;
            var paginatedList = await PaginatedList<AuthorSong>.CreateAsync(applicationDbContext.AsNoTracking(), page, pageSize);
        ...
        public async Task<CommonSingleViewModel> GetSingleAuthorSong(string slug, int page, int pageSize)
        {
