...
/**
 * A jQuery plugin (sic) to transpose the key in song sheets.
...
/**
 * A Module for transposing music in HTML generated from chord pro with chopro2html.py
 * You will need elb.js to establish the global ELB,
 * The transposer jQuery plugin above,
 * and jQuery.
 */
...
/**
 * Initialize this transposer.
 * @param theKeyElement where the transposition buttons will be added.
 */
ELB.Music.aTransposer = function (theKeyElement) {

    var newTransposer = {

        /**
         * add a couple buttons for transposing up and down half steps.
         * @param theHostElement a DOM elements.
         *        The buttons will straddle the inner HTML of this element.
         */
        addButtons: function (theHostElement) {
            var prevHTML, ourButtons;
            prevHTML = jQuery(theHostElement).html();
            jQuery(theHostElement).html('<input class="tbutton btn btn-default btn-sm" type="button" value="-">{prevHTML}<input class="tbutton btn btn-default btn-sm" type="button" value="+">'.supplant({prevHTML: prevHTML}));
            ourButtons = jQuery('input.tbutton');
            jQuery(ourButtons[0]).click(function (theEvent) {
                $('.chord,.key').elb_music_transpose(-1); // transpose down.
            });
            jQuery(ourButtons[1]).click(function (theEvent) {
                $('.chord,.key').elb_music_transpose(1); // transpose up.
            });
        },

        /**
         * Add a listener so that when the key changes
         * they will get notified via their setKey() method.
         * Note that this listener's initial key should be set independently.
         * We could set it here if we stopped setting it when creating the listener.
         */
        addListener: function (theListener) {
            $('.tbutton').click(function (theEvent) {
                var ourKey = $(theKeyElement).find('span').html();
                theListener.setKey(ourKey);
            });
        }
    ...
    if (theKeyElement) {
        newTransposer.addButtons(theKeyElement);
    }

    return newTransposer;

};

/**
 * Automatically initializes a transposer
 * if an element with id="musicalKey" can be found
 * (hopefully from the "key" directive in the original chopro file).
jQuery(document).ready(function () {
    var ourTransposer, k = jQuery('#musicalKey');
    if (k) {
        ourTransposer = ELB.Music.aTransposer(k);
    }
});
 */
